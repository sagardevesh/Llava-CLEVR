...
class CustomDataset(Dataset):
    """
    Custom dataset for fine-tuning on the BiCLEVR dataset.
    Only uses Camera1 images.
    """
    def __init__(self, images_dir, questions_file, processor, max_images=None, split="train"):
        ...
        # Filter questions to only include Camera1 images
        camera1_questions = [q for q in questions_list if "Camera1" in q["image_filename"]]
        
        image_to_qas = defaultdict(list)
        for q in camera1_questions:
        ...
        for fn in used:
            image_path = os.path.join(self.images_dir, fn)
...
def main():
    ...
    parser.add_argument("--lr", type=float, default=1e-5)  # Changed to 1e-5
    parser.add_argument("--save_dir", type=str, default="custom_lora_out")
    ...
    print(f"Created dataset with {len(train_dataset)} training samples and {len(val_dataset)} validation samples")
